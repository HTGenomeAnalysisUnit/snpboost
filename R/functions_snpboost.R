...
predict_snpboost <- function(fit, new_genotype_file, new_phenotype_file, phenotype, subset=NULL,idx=NULL){
  ...
  if(nrow(chosen_SNPs)>0){
    plink2_cmd <- paste(fit$configs['plink2.path'],"--pfile",new_genotype_file,ifelse(configs[['vzs']], 'vzs', ''),"--score",
...
computeMetric <- function(residual, phenotype, prediction, configs, phenotype2 = NULL, weights = NULL, sigma= NULL, T_order = NULL) {
  if (configs[['metric']] == 'MSEP') {
    ...
    sum_cindex <- function(i){ifelse(i<length(prediction),wweights[T_order][i]*sum(approxLoss(prediction[T_order][(i+1):length(prediction)]-prediction[T_order][i])[(phenotype[,1][T_order][(i+1):length(prediction)]-phenotype[,1][T_order][i])!=0]),0)}
    
    metric=-parallel::mcmapply(sum_cindex,1:length(prediction),mc.cores = configs[['nCores']])
    
    #metric <- -survAUC::UnoC(phenotype, phenotype2, prediction)
    #for now: not cencsoring corrected because it doesn't work and I don't know why
    #metric <- -survival::concordance(phenotype~prediction)$concordance
  } else if(configs[['metric']] == 'weightedL2'){
    time <- phenotype[,1]
    ### log(0) will produce NaN
    time[time==0] <- sort(unique(time))[2]/10
...
checkEarlyStoppingBatches <- function(metric.val, iter, b_stop, configs, m_batch_list){
  if(iter <= b_stop){
   ...
   # if(configs[['metric']]=='MSEP' || configs[['metric']]=='log_loss' || configs[['metric']]=='C' || configs[['metric']]=='quantilereg'){
...
setupConfigs <- function(configs, genotype.pfile, phenotype.file, phenotype, covariates, split.col, status.col, mem){
  ...
  if (file.exists(paste0(genotype.pfile, '.pvar'))) { out[['vzs']] <- FALSE }
  out
}
...
computeStats <- function(pfile, ids, configs) {
  ...
  if (file.exists(gcount_tsv_f)) {
    ...
    data.table::fwrite(ids, file=keep_f, sep='\t', col.names=F)
