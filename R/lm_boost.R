lm_boost <- function(y,x,validation=FALSE,y_val=NULL,x_val=NULL,beta=data.frame(name=NA,intercept=0,variant=NA),m_batch=1,sl=0.1,
  ...
  for(m in 1:m_batch){
   #find highest correlation
    ...
    if(configs[['family']]=="cox"){
      if(m>1 & all(order(pred)==pred_tmp) & (!configs[['metric']] %in% c("weightedL2","AFT-Weibull","AFT-logistic","AFT-normal","Cox"))){
        metric.train[m] = metric.train[m-1]
      }else{
        metric.train[m] = computeMetric(r, surv[['train']], pred, configs, surv[['train']], IPCw[['train']],sigma)
        pred_tmp <- order(pred)
      }
    }else{
    ...
    if(validation){
      ...
      if(configs[['family']]=="cox"){
        if(m>1 & all(order(pred.val)==pred.val_tmp) & (!configs[['metric']] %in% c("weightedL2","AFT-Weibull","AFT-logistic","AFT-normal","Cox"))){
